...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      // Brief pause between batches to respect rate limits (1s since we send small page-split PDFs)
      if (i > 0) {
...
function createPageBatches(
  ...
  if (pageGroups.length > 0) {
    for (const group of pageGroups) {
      ...
      // Use batches of up to 8 pages to reduce total API calls and avoid browser timeouts
      for (let i = 0; i < pages.length; i += 8) {
        const batchPages = pages.slice(i, i + 8);
